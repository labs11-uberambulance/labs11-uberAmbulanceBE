exports.up = function(knex, Promise) {
    return knex.schema.createTable('rides', function(tbl){
        tbl.increments('id');
        tbl.integer('driver_id').unsigned().notNullable().references('id').inTable('drivers').onUpdate('CASCADE');
        tbl.integer('mother_id').unsigned().notNullable().references('id').inTable('mothers').onUpdate('CASCADE');
